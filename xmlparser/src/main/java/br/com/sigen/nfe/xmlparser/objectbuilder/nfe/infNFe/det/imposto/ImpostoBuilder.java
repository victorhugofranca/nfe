...
package br.com.sigen.nfe.xmlparser.objectbuilder.nfe.infNFe.det.imposto;
	...
	public Imposto build(Map<String, Object> map) throws ObjectBuilderException {
		...
		imposto.getContent().add(
				objectFactory.createTNFeInfNFeDetImpostoICMS(icmsBuilder
						.build(map)));
		imposto.getContent().add(
				objectFactory.createTNFeInfNFeDetImpostoII(iiBuilder.build(map)));
		imposto.getContent()
				.add(objectFactory.createTNFeInfNFeDetImpostoIPI(ipiBuilder
						.build(map)));
		imposto.getContent().add(
				objectFactory.createTNFeInfNFeDetImpostoISSQN(issqnBuilder
						.build(map)));
		imposto.getContent()
				.add(objectFactory.createTNFeInfNFeDetImpostoPIS(pisBuilder
						.build(map)));
		imposto.getContent().add(
